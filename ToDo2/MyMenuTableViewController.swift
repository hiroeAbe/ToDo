...
class MyMenuTableViewController: UITableViewController {
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        if (cell == nil) {
            ...
            selectedBackgroundView.backgroundColor = UIColor.purpleColor().colorWithAlphaComponent(0.2)
            cell!.selectedBackgroundView = selectedBackgroundView
        }
        
       // cell!.textLabel?.text = "mein\(indexPath.row+1)"
        if(indexPath.row==0){
            cell!.textLabel?.text="Main"
        }else if(indexPath.row==1){
            cell!.textLabel?.text="List"
        }else if(indexPath.row==2){
            cell!.textLabel?.text="Calender"
        }
    ...
    override func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        case 2:
            ...
            break
        case 1:
            destViewController = mainStoryboard.instantiateViewControllerWithIdentifier("TabBarController") as UITabBarController
            break
        default:
